...
def writeTestBench(input_file, root_path):
    ...
    # 
    sb_inst = "scoreboard_dut %s u_scoreboard_dut (\n%s\n);" %(paraDef,portList)

    # write testbench
    tmp_path = root_path + "/script/py/lib/tb_top.sv.tmp"
    with open(tmp_path,"r") as f:
        tb_tmp = f.read()
    f.close()
    tb_code = tb_tmp.format(signal_dec=signal_dec,top_inst=top_inst,sb_inst=sb_inst)
...
def writeSB(input_file):
    ...
    portList = findPort(inText)
    # input  = formatDeclare(input ,'reg', '0' )
    # output = formatDeclare(output ,'wire')
    # inout  = formatDeclare(inout ,'wire')

    # TODO logic 支持, 二维数组支持
    signal_dec = formatDeclare(portList ,'logic')
    # UUT
    sb_code = "scoreboard_dut %s u_scoreboard_dut (\n%s\n);" %(paraList,portList)
